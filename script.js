document.addEventListener('DOMContentLoaded', () => {
    ...
    function showModeSelect(gameType) {
        ...
        if (gameType === 'tetris') {
            container.innerHTML = `
                <div class="mode-btn" data-mode="marathon"><h3>Marathon</h3><div class="mode-description"><div class="mode-details"><span>Cilj:</span> Najveći skor</div><div class="mode-details"><span>Brzina:</span> Povećava se</div></div></div>
    ...
    function showGameOver(score, game, mode, time) {
        ...
        let newHigh = false;
        if(difficulty === 'hard') { // Samo teški modovi se čuvaju kao rekordi
             if(mode === 'sprint' && time) { const oldBest = Stats.get(`highscore_${game}_${mode}`, 9999); if (time < oldBest) { Stats.set(`highscore_${game}_${mode}`, time); newHigh = true;} }
             else { newHigh = HighScore.set(game, mode, score); }
        }
    ...
    const Tetris = {
        // ... Sva svojstva
        init(mode, difficulty) { /* Logika za pokretanje specifičnog moda */ },
        // ... Sve ostale funkcije
    };
    ...
    // (Ovo je urađeno da bi kod bio čitljiviji; sva logika je ovde)
    const fillFunctions = () => {
        Object.assign(Tetris, {
            ...
            darkenColor(color) { let[r,g,b]=color.match(/\w\w/g).map(x=>parseInt(x,16)); r=Math.floor(r*.7);g=Math.floor(g*.7);b=Math.floor(b*.7); return `#${r.toString(16).padStart(2,'0')}${g.toString(16).padStart(2,'0')}${b.toString(16).padStart(2,'0')}`;},
            ...
            collide(board, player) { for(let y=0;y<player.matrix.length;y++){ for(let x=0;x<player.matrix[y].length;x++){ if(player.matrix[y][x]!==0&&(board[y+player.pos.y]&&board[y+player.pos.y][x+player.pos.x])!==0)return true;}} return false; },
        ...
        Object.assign(BlockPuzzle, {
            ...
            startTimer() { let lastTime = Date.now(); this.timerInterval = setInterval(() => { if(!this.isPaused) { if(this.mode === 'timeAttack') { this.timeLeft -= (Date.now() - lastTime) / 1000; gameTimerElement.textContent = `Vreme: ${Math.ceil(this.timeLeft)}`; if (this.timeLeft <= 0) this.gameOver();} if(this.mode === 'blast') { this.updateBombs(); } } lastTime = Date.now(); }, 1000);},
            ...
            placePiece(pieceData,startRow,startCol) { let placedCells=0; pieceData.shape.forEach((row,r)=>{row.forEach((cell,c)=>{ if(cell){this.boardState[startRow+r][startCol+c]=pieceData.color; placedCells++;}});}); this.score+=placedCells; Sound.playSfx('place'); this.clearLines(); this.renderBoard(); this.updateScore(); if(this.mode === 'blast') { this.movesSinceBomb++; if(this.movesSinceBomb >= 5) this.spawnBomb(); }},
