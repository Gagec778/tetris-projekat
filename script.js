document.addEventListener('DOMContentLoaded', () => {
    ...
    const Stats = {
        ...
        increment(key) { localStorage.setItem(key, this.get(key, 0) + 1); },
        add(key, value) { localStorage.setItem(key, this.get(key, 0) + value); },
        render() {
            const container = document.querySelector('.stats-container');
            const fastestSprint = this.get('tetris_fastestSprint', 'N/A');
            container.innerHTML = `
                <div class="stats-group">
                    <h3>Classic Blocks</h3>
                    ...
                    <div class="stat-item"><span>Najbrži Sprint:</span><span class="value">${fastestSprint !== 'N/A' ? fastestSprint + ' s' : 'N/A'}</span></div>
                </div>
                <div class="stats-group">
                    <h3>Puzzle Board</h3>
    ...
    function showModeSelect(gameType) {
        ...
        document.getElementById('mode-select-title').textContent = gameType === 'tetris' ? 'Izaberi Mod' : 'Izaberi Mod';
        if (gameType === 'tetris') {
            container.innerHTML = `
    ...
    function showGameOver(score, game, mode, time) {
        ...
        if (mode === 'sprint' && time) {
            ...
            if (time < oldBest) { Stats.set(`tetris_fastestSprint`, time); newHigh = true;}
        }
    ...
    function animateScore(finalScore) { let currentScore=0; const inc=Math.max(1,Math.floor(finalScore/100)); const int=setInterval(()=>{currentScore+=inc; if(currentScore>=finalScore){currentScore=finalScore;clearInterval(int);} finalScoreElement.textContent=currentScore;},10); }
    ...
    helpBtn.addEventListener('click', () => { helpBlockPuzzleContent.classList.toggle('active', activeGame === 'block-puzzle'); helpTetrisContent.classList.toggle('active', activeGame === 'tetris'); helpModal.classList.add('active'); });
    ...
    const Tetris = {
        ...
        mode: 'classic', difficulty: 'normal', combo: 0, linesCleared: 0, timer: 0, timerInterval: null,
        ...
        init(mode, difficulty) {
            ...
            this.combo = 0; this.linesCleared = 0; this.updateScore();
            ...
            gameTimerElement.textContent = '';
            if(this.mode === 'sprint') gameTimerElement.textContent = `Linije: ${40 - this.linesCleared}`;
            this.addListeners();
            this.resume();
        },
        pause() { this.isPaused = true; this.isRunning = false; if(this.timerInterval) clearInterval(this.timerInterval); },
        resume() { this.isPaused = false; this.isRunning = true; this.lastTime = performance.now(); this.animationFrameId = requestAnimationFrame(this.update.bind(this)); if(this.mode === 'sprint') this.startTimer();},
        startTimer() {
            let startTime = Date.now() - this.timer * 1000;
            this.timerInterval = setInterval(() => { this.timer = ((Date.now() - startTime) / 1000).toFixed(2); }, 100);
        },
        ...
        draw() { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        darkenColor(color) { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        createMatrix(w, h) { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        createPiece() { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        collide(board, player) { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        merge() { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        arenaSweep() {
            let clearedRows = [];
            ...
            if (clearedRows.length > 0) {
                ...
                this.player.score += ([0, 10, 30, 50, 100][clearedRows.length] + (this.combo * 5)) * 10;
                this.updateScore();
                Sound.playSfx('clear');
                Stats.add('tetris_linesCleared', clearedRows.length);
                clearedRows.forEach(y_index => { this.playLineClearEffect(y_index); this.boardState.splice(y_index, 1); this.boardState.unshift(new Array(this.cols).fill(0)); });
                if (this.mode === 'classic' && clearedRows.length > 0) this.dropInterval *= Math.pow(0.98, clearedRows.length);
                if (this.mode === 'sprint') { gameTimerElement.textContent = `Linije: ${40 - this.linesCleared}`; if(this.linesCleared >= 40) this.gameOver(); }
            } else { this.combo = 0; }
        },
        playLineClearEffect(rowIndex) { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        updateScore() { scoreElement.textContent = this.player.score; },
        playerReset() { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        playerMove(dir) { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        playerDrop() { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        rotate(m,d) { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        playerRotate(dir) { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        handleKeyDown(e) { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        handleTouchStart(e) { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        handleTouchMove(e) { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        handleTouchEnd() { /* CEO KOD IZ PRETHODNOG ODGOVora */ },
        hardDrop() { /* CEO KOD IZ PRETHODNOG ODGOVORA */ },
        gameOver() { this.stop(); Sound.playSfx('gameOver'); let finalValue = this.mode === 'sprint' ? parseFloat(this.timer) : this.player.score; showGameOver(finalValue, 'tetris', this.mode, this.mode === 'sprint' ? finalValue : null); }
    };
    ...
    const BlockPuzzle = {
        // ... Sva svojstva
        isRunning: false, isPaused: false, mode: 'classic', difficulty: 'normal', combo: 0, timeLeft: 120, timerInterval: null,
        init(mode, difficulty) {
            this.mode = mode; this.difficulty = difficulty;
            ...
            this.boardState=Tetris.createMatrix(10,10); this.score=0; this.updateScore();
            ...
            if (this.timerInterval) clearInterval(this.timerInterval);
            gameTimerElement.textContent = '';
            if (this.mode === 'timeAttack') { this.timeLeft = 120; }
            this.resume();
        },
        pause() { this.isPaused = true; this.isRunning = false; if(this.timerInterval) clearInterval(this.timerInterval); },
        ...
        startTimer() {
            ...
            this.timerInterval = setInterval(() => {
                if(!this.isPaused) {
                    this.timeLeft -= (Date.now() - lastTime) / 1000;
                    lastTime = Date.now();
                }
                gameTimerElement.textContent = `Vreme: ${Math.ceil(this.timeLeft)}`;
                if (this.timeLeft <= 0) { gameTimerElement.textContent = 'Vreme: 0'; clearInterval(this.timerInterval); this.gameOver(); }
            }, 100);
        },
        ...
        clearLines() {
            ...
            if (clearedCount > 0) {
                ...
                Sound.playSfx('clear'); Stats.add('blockPuzzle_linesCleared', clearedCount);
            } else { this.combo = 0; }
        },
    ...
    // Zbog ogromne dužine, tela funkcija koje se nisu menjale su skraćena.
    // Ovde je kompletan kod za njih da biste imali sve na jednom mestu.
    const fillFunctions = () => {
        Tetris.draw = function() { this.board.innerHTML = ''; const fullGrid = JSON.parse(JSON.stringify(this.boardState)); const ghost = {...this.player, matrix: JSON.parse(JSON.stringify(this.player.matrix)) }; while (!this.collide(this.boardState, ghost)) ghost.pos.y++; ghost.pos.y--; ghost.matrix.forEach((row, y) => { row.forEach((value, x) => { if (value !== 0) { const r = y + ghost.pos.y; const c = x + ghost.pos.x; if(r >= 0 && r < this.rows && c >= 0 && c < this.cols) fullGrid[r][c] = 'ghost'; } }); }); this.player.matrix.forEach((row, y) => { row.forEach((value, x) => { if (value !== 0) { const r = y + this.player.pos.y; const c = x + this.player.pos.x; if(r >= 0 && r < this.rows && c >= 0 && c < this.cols) fullGrid[r][c] = value; }}); }); fullGrid.forEach(row => { row.forEach(value => { const cell = document.createElement('div'); if (typeof value === 'number' && value !== 0) { cell.className = 'game-block'; const color = this.colors[value]; cell.style.background = `linear-gradient(145deg, ${color}, ${this.darkenColor(color)})`; cell.style.setProperty('--glow-color', color); } else if (value === 'ghost') { cell.className = 'tetris-cell ghost'; } this.board.appendChild(cell); }); }); this.nextContainer.innerHTML = ''; this.nextPieces.forEach(pieceMatrix => { const display = document.createElement('div'); display.className = 'next-piece-display'; display.style.gridTemplateColumns = `repeat(${pieceMatrix[0].length}, 1fr)`; pieceMatrix.forEach(row => { row.forEach(value => { const cell = document.createElement('div'); cell.style.width = '15px'; cell.style.height = '15px'; if (value !== 0) { cell.className = 'game-block'; const color = this.colors[value]; cell.style.background = `linear-gradient(145deg, ${color}, ${this.darkenColor(color)})`; cell.style.setProperty('--glow-color', color); } display.appendChild(cell); }); }); this.nextContainer.appendChild(display); }); };
        Tetris.darkenColor = function(color) { let[r,g,b]=color.match(/\w\w/g).map(x=>parseInt(x,16)); r=Math.floor(r*.7);g=Math.floor(g*.7);b=Math.floor(b*.7); return `#${r.toString(16).padStart(2,'0')}${g.toString(16).padStart(2,'0')}${b.toString(16).padStart(2,'0')}`;};
        Tetris.createMatrix = function(w, h) { const m = []; while(h--) m.push(new Array(w).fill(0)); return m; };
        Tetris.createPiece = function() { const p='TJLOSZI'[Math.floor(Math.random()*7)]; switch(p){ case 'T':return [[1,1,1],[0,1,0]]; case 'J':return [[2,0,0],[2,2,2]]; case 'L':return [[0,0,3],[3,3,3]]; case 'O':return [[4,4],[4,4]]; case 'S':return [[0,5,5],[5,5,0]]; case 'Z':return [[6,6,0],[0,6,6]]; case 'I':return [[0,0,0,0],[7,7,7,7],[0,0,0,0]]; }};
        Tetris.collide = function(board, player) { for(let y=0;y<player.matrix.length;y++){ for(let x=0;x<player.matrix[y].length;x++){ if(player.matrix[y][x]!==0&&(board[y+player.pos.y]&&board[y+player.pos.y][x+player.pos.x])!==0)return true;}} return false; };
        Tetris.merge = function() { this.player.matrix.forEach((row, y) => { row.forEach((value, x) => { if (value !== 0) { this.boardState[y + this.player.pos.y][x + this.player.pos.x] = value; const cellIndex = (y + this.player.pos.y) * this.cols + (x + this.player.pos.x); if(this.board.children[cellIndex]) this.board.children[cellIndex].classList.add('lock-in'); } }); }); Sound.playSfx('place'); };
        Tetris.playLineClearEffect = function(rowIndex) { for (let i = 0; i < this.cols; i++) { const cellIndex = rowIndex * this.cols + i; if(this.board.children[cellIndex])this.board.children[cellIndex].classList.add('line-clearing');}};
        Tetris.playerReset = function() { this.player.matrix = this.nextPieces.shift(); this.nextPieces.push(this.createPiece()); this.player.pos.y = 0; this.player.pos.x = Math.floor(this.cols/2)-Math.floor(this.player.matrix[0].length/2); if(this.collide(this.boardState,this.player)) this.gameOver(); };
        Tetris.playerMove = function(dir) { this.player.pos.x += dir; if(this.collide(this.boardState,this.player)) this.player.pos.x-=dir; };
        Tetris.playerDrop = function() { this.player.pos.y++; if (this.collide(this.boardState, this.player)) { this.player.pos.y--; this.merge(); this.playerReset(); this.arenaSweep(); } this.dropCounter = 0; };
        Tetris.rotate = function(m,d) { for(let y=0;y<m.length;++y){for(let x=0;x<y;++x)[m[x][y],m[y][x]]=[m[y][x],m[x][y]];} d>0?m.forEach(r=>r.reverse()):m.reverse(); };
        Tetris.playerRotate = function(dir) { const pos = this.player.pos.x; let offset=1; this.rotate(this.player.matrix,dir); while(this.collide(this.boardState,this.player)){ this.player.pos.x+=offset; offset=-(offset+(offset>0?1:-1)); if(offset>this.player.matrix[0].length){this.rotate(this.player.matrix,-dir);this.player.pos.x=pos;return;}}};
        Tetris.handleKeyDown = function(e) { if(!this.isRunning) return; switch(e.keyCode){ case 37:this.playerMove(-1);break; case 39:this.playerMove(1);break; case 40:this.playerDrop();break; case 81:this.playerRotate(-1);break; case 87:case 38:this.playerRotate(1);break; case 32:this.hardDrop();break; }};
        Tetris.handleTouchStart = function(e) { e.preventDefault(); this.touchStartX=e.touches[0].clientX; this.touchStartY=e.touches[0].clientY; this.touchMoveX=this.touchStartX; this.touchMoveY=this.touchStartY; this.boundTouchMove=this.handleTouchMove.bind(this); this.boundTouchEnd=this.handleTouchEnd.bind(this); this.board.addEventListener('touchmove',this.boundTouchMove,{passive:false}); this.board.addEventListener('touchend',this.boundTouchEnd,{passive:false});};
        Tetris.handleTouchMove = function(e) { e.preventDefault(); this.touchMoveX=e.touches[0].clientX; this.touchMoveY=e.touches[0].clientY; };
        Tetris.handleTouchEnd = function() { this.board.removeEventListener('touchmove',this.boundTouchMove); this.board.removeEventListener('touchend',this.boundTouchEnd); const dX=this.touchMoveX-this.touchStartX; const dY=this.touchMoveY-this.touchStartY; const bW=this.board.clientWidth/this.cols; if(Math.abs(dX)>Math.abs(dY)){if(Math.abs(dX)>bW/2)this.playerMove(dX>0?1:-1);} else {if(dY>bW*2)this.hardDrop();else if(dY>bW/2)this.playerDrop();else if(Math.abs(dX)<20&&Math.abs(dY)<20)this.playerRotate(1);}};
        Tetris.hardDrop = function() { while(!this.collide(this.boardState,this.player))this.player.pos.y++; this.player.pos.y--; this.merge(); this.playerReset(); this.arenaSweep(); shakeScreen(); Sound.playSfx('hardDrop'); };

        BlockPuzzle.renderBoard = function() { this.boardElement.innerHTML=''; this.boardState.forEach(row=>{row.forEach(cellValue=>{ const cell=document.createElement('div'); cell.className='puzzle-cell'; if(cellValue){const color=cellValue;cell.style.background=`linear-gradient(145deg, ${color}, ${Tetris.darkenColor(color)})`; cell.style.setProperty('--glow-color', color); cell.classList.add('filled','game-block');} this.boardElement.appendChild(cell); });});};
        BlockPuzzle.generateNewPieces = function() { this.currentPieces=[]; this.piecesContainer.innerHTML=''; for(let i=0;i<3;i++){ const pieceData={...this.pieceShapes[Math.floor(Math.random()*this.pieceShapes.length)]}; pieceData.id=`piece-${Date.now()}-${i}`; this.currentPieces.push(pieceData); this.renderPiece(pieceData); } if(this.checkGameOver()){this.gameOver();}};
        BlockPuzzle.renderPiece = function(pieceData) { const pieceEl=document.createElement('div'); pieceEl.className='puzzle-piece'; pieceEl.dataset.id=pieceData.id; pieceEl.style.gridTemplateColumns=`repeat(${pieceData.shape[0].length},1fr)`; pieceData.shape.forEach(row=>{row.forEach(cell=>{ const cellEl=document.createElement('div'); if(cell){cellEl.className='puzzle-piece-cell game-block'; const color=pieceData.color; cellEl.style.background=`linear-gradient(145deg, ${color}, ${Tetris.darkenColor(color)})`; cellEl.style.setProperty('--glow-color', color);} pieceEl.appendChild(cellEl); });}); this.piecesContainer.appendChild(pieceEl); };
        BlockPuzzle.onDragStart = function(e) { if(!e.target.closest('.puzzle-piece')||!this.isRunning) return; e.preventDefault(); const pieceEl=e.target.closest('.puzzle-piece'); const pieceId=pieceEl.dataset.id; const pieceData=this.currentPieces.find(p=>p.id===pieceId); this.activeDragPiece=pieceData; const draggingEl=pieceEl.cloneNode(true); draggingEl.classList.add('dragging'); document.body.appendChild(draggingEl); const rect=pieceEl.getBoundingClientRect(); const touch=e.type==='touchstart'?e.touches[0]:e; const offsetX=touch.clientX-rect.left; const offsetY=touch.clientY-rect.top; pieceEl.style.opacity='0.3'; const moveHandler=(moveEvent)=>{ const moveTouch=moveEvent.type==='touchmove'?moveEvent.touches[0]:moveEvent; draggingEl.style.left=`${moveTouch.clientX-offsetX}px`; draggingEl.style.top=`${moveTouch.clientY-offsetY}px`; this.renderGhost(pieceData,moveTouch); }; const endHandler=(endEvent)=>{ document.removeEventListener('mousemove',moveHandler); document.removeEventListener('touchmove',moveHandler); document.removeEventListener('mouseup',endHandler); document.removeEventListener('touchend',endHandler); draggingEl.remove(); this.boardElement.querySelectorAll('.ghost-path').forEach(c=>{c.classList.remove('ghost-path');c.style.backgroundColor='';}); this.activeDragPiece=null; const boardRect=this.boardElement.getBoundingClientRect(); const endTouch=endEvent.type==='touchend'?endEvent.changedTouches[0]:endEvent; const x=endTouch.clientX-boardRect.left; const y=endTouch.clientY-boardRect.top; const col=Math.floor(x/(boardRect.width/10)); const row=Math.floor(y/(boardRect.height/10)); if(this.canPlacePiece(pieceData,row,col)){ this.placePiece(pieceData,row,col); pieceEl.remove(); this.currentPieces=this.currentPieces.filter(p=>p.id!==pieceId); if(this.currentPieces.length===0)this.generateNewPieces(); } else {pieceEl.style.opacity='1';} }; document.addEventListener('mousemove',moveHandler); document.addEventListener('touchmove',moveHandler); document.addEventListener('mouseup',endHandler); document.addEventListener('touchend',endHandler); };
        BlockPuzzle.renderGhost = function(pieceData,moveTouch) { this.boardElement.querySelectorAll('.ghost-path').forEach(c=>{c.classList.remove('ghost-path');c.style.backgroundColor='';}); if(!pieceData) return; const boardRect=this.boardElement.getBoundingClientRect(); const x=moveTouch.clientX-boardRect.left; const y=moveTouch.clientY-boardRect.top; const startCol=Math.floor(x/(boardRect.width/10)); const startRow=Math.floor(y/(boardRect.height/10)); for(let r=0;r<pieceData.shape.length;r++){for(let c=0;c<pieceData.shape[r].length;c++){ if(pieceData.shape[r][c]){ const bR=startRow+r; const bC=startCol+c; if(bR<10&&bC<10&&bR>=0&&bC>=0){ const cellIndex=bR*10+bC; const cell=this.boardElement.children[cellIndex]; if(cell&&!this.boardState[bR][bC]){ cell.classList.add('ghost-path'); cell.style.setProperty('--color',pieceData.color);}} }}}};
        BlockPuzzle.canPlacePiece = function(p,startRow,startCol){for(let r=0;r<p.shape.length;r++){for(let c=0;c<p.shape[r].length;c++){if(p.shape[r][c]){const bR=startRow+r;const bC=startCol+c;if(bR>=10||bC>=10||bR<0||bC<0||this.boardState[bR][bC])return false;}}}return true;};
        BlockPuzzle.placePiece = function(pieceData,startRow,startCol) { let placedCells=0; pieceData.shape.forEach((row,r)=>{row.forEach((cell,c)=>{ if(cell){this.boardState[startRow+r][startCol+c]=pieceData.color; placedCells++;}});}); this.score+=placedCells; Sound.playSfx('place'); this.clearLines(); this.renderBoard(); this.updateScore(); };
        BlockPuzzle.checkGameOver = function() { for(const piece of this.currentPieces){ for(let r=0;r<=10-piece.shape.length;r++){ for(let c=0;c<=10-piece.shape[0].length;c++){ if(this.canPlacePiece(piece,r,c))return false;}}} return true; };
        BlockPuzzle.updateScore = function() { scoreElement.textContent = this.score; };
    }
    fillFunctions();
});
