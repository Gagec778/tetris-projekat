document.addEventListener('DOMContentLoaded', () => {
    ...
    const Tetris = {
        ...
        draw() {
            ...
            fullGrid.forEach(row => {
                row.forEach(value => {
                    ...
                    } else if (value === 'ghost') { cell.className = 'tetris-cell ghost'; }
        ...
        createPiece() {
            ...
            switch(p){ case 'T':return [[1,1,1],[0,1,0]]; case 'J':return [[2,0,0],[2,2,2],[0,0,0]]; case 'L':return [[0,0,3],[3,3,3],[0,0,0]];
                case 'O':return [[4,4],[4,4]]; case 'S':return [[0,5,5],[5,5,0]]; case 'Z':return [[6,6,0],[0,6,6]];
            ...
            this.player.matrix.forEach((row, y) => {
                row.forEach((value, x) => {
                    if (value !== 0) {
                        ...
                        if(this.board.children[cellIndex]) this.board.children[cellIndex].classList.add('lock-in');
                    }
                });
            });
            Sound.play('place');
    ...
    const BlockPuzzle = {
        ...
        onDragStart(e) {
            ...
            const moveHandler=(moveEvent)=>{ const moveTouch=moveEvent.type==='touchmove'?moveEvent.touches[0]:moveEvent; draggingEl.style.left=`${moveTouch.clientX-offsetX}px`; draggingEl.style.top=`${moveTouch.clientY-offsetY}px`; this.renderGhost(pieceData,moveTouch); };
            const endHandler=(endEvent)=>{
                ...
                draggingEl.remove(); this.boardElement.querySelectorAll('.ghost-path').forEach(c=>{c.classList.remove('ghost-path');c.style.backgroundColor='';}); this.activeDragPiece=null;
                const boardRect=this.boardElement.getBoundingClientRect(); const endTouch=endEvent.type==='touchend'?endEvent.changedTouches[0]:endEvent;
                ...
                if(this.canPlacePiece(pieceData,row,col)){
                    this.placePiece(pieceData,row,col); pieceEl.remove(); this.currentPieces=this.currentPieces.filter(p=>p.id!==pieceId);
        ...
        placePiece(pieceData,startRow,startCol) {
            let placedCells=0;
            pieceData.shape.forEach((row,r)=>{row.forEach((cell,c)=>{ if(cell){this.boardState[startRow+r][startCol+c]=pieceData.color; placedCells++;}});});
        ...
        clearLines() {
            ...
            if(clearedCount>0){this.score+=clearedCount*clearedCount*10;Sound.play('clear');}
        },
