document.addEventListener('DOMContentLoaded', () => {
    ...
    const tetrisUI = {
        header: document.getElementById('tetris-ui-header'),
    ...
    const Sound = {
        ...
        init() {
            try {
                ...
                this.sfx.bomb = new Audio('assets/sounds/bomb.mp3');
                ...
                console.warn("Greška pri učitavanju zvuka. Proverite da li 'assets/sounds/' folder postoji.");
            }
        },
    ...
    const Settings = {
        ...
        setHaptics(enabled) {
            Sound.hapticsEnabled = enabled;
            localStorage.setItem('puzzleHaptics', enabled);
            this.hapticsBtn.textContent = enabled ? 'ON' : 'OFF';
            this.hapticsBtn.classList.toggle('active', enabled);
        }
    };
    ...
    const Stats = {
        ...
        render() {
            ...
            const fastestSprint = this.get('highscore_tetris_sprint_hard', 'N/A');
            container.innerHTML = `
                <div class="stats-group">
                    ...
                    <div class="stat-item"><span>Najbolji Ultra Skor (Teško):</span><span class="value">${this.get('highscore_tetris_ultra_hard')}</span></div>
                </div>
                <div class="stats-group">
                    <h3>Puzzle Board</h3>
                    ...
                    <div class="stat-item"><span>Najbolji Time Attack (Teško):</span><span class="value">${this.get('highscore_block-puzzle_timeAttack_hard')}</span></div>
                    <div class="stat-item"><span>Najbolji Blast Skor (Teško):</span><span class="value">${this.get('highscore_block-puzzle_blast_hard')}</span></div>
                </div>`;
        }
    };
    const HighScore = {
        get(game, mode, difficulty) { return parseFloat(localStorage.getItem(`highscore_${game}_${mode}_${difficulty}`) || '0'); },
        ...
        updateDisplay() {
            highscoreElements['tetris-marathon'].textContent = this.get('tetris', 'marathon', 'hard');
            highscoreElements['block-puzzle-classic'].textContent = this.get('block-puzzle', 'classic', 'hard');
        }
    };
    ...
    const MainMenu3D = {
        ...
        onMouseMove(e) {
            ...
            this.items.forEach(item => {
                item.style.setProperty('--rotate-y', `${x * 20}deg`);
                item.style.setProperty('--rotate-x', `${-y * 20}deg`);
            });
        }
    };
    ...
    function showScreen(screenName) {
        ...
        if (screenName === 'mainMenu') MainMenu3D.init();
    ...
    function showModeSelect(gameType) {
        ...
        if (gameType === 'tetris') {
            container.innerHTML = `
                <div class="mode-btn" data-mode="marathon">
                    <h3>Marathon (Classic)</h3>
                    <div class="mode-description">
                        <div class="mode-details"><span>Cilj:</span> Najveći skor</div>
                        <div class="mode-details"><span>Brzina:</span> Postepeno se povećava</div>
                    </div>
                </div>
                <div class="mode-btn" data-mode="sprint">
                    <h3>Sprint</h3>
                    <div class="mode-description">
                        <div class="mode-details"><span>Cilj:</span> Očisti 40 linija</div>
                        <div class="mode-details"><span>Skor:</span> Vreme</div>
                    </div>
                </div>
                <div class="mode-btn" data-mode="ultra">
                    <h3>Ultra</h3>
                    <div class="mode-description">
                        <div class="mode-details"><span>Cilj:</span> Najveći skor</div>
                        <div class="mode-details"><span>Vreme:</span> 3:00</div>
                    </div>
                </div>`;
        } else {
            container.innerHTML = `
                <div class="mode-btn" data-mode="classic">
                    ...
                    <div class="mode-description">
                        <div class="mode-details"><span>Cilj:</span> Najveći skor</div>
                        <div class="mode-details"><span>Pravilo:</span> Bez žurbe</div>
                    </div>
                </div>
                <div class="mode-btn" data-mode="timeAttack">
                    ...
                    <div class="mode-description">
                        <div class="mode-details"><span>Cilj:</span> Najveći skor</div>
                        <div class="mode-details"><span>Vreme:</span> 2:00</div>
                    </div>
                </div>
                <div class="mode-btn" data-mode="blast">
                    <h3>Blast</h3>
                    <div class="mode-description">
                        <div class="mode-details"><span>Cilj:</span> Preživi</div>
                        <div class="mode-details"><span>Pravilo:</span> Pazi na bombe!</div>
                    </div>
                </div>`;
        }
        showScreen('gameMode');
    ...
    async function showFirstTimeHelp(game) {
        ...
        if (!localStorage.getItem(key)) {
            document.getElementById('help-bubble-score').style.display = game === 'tetris' ? 'none' : 'block';
    ...
    async function runCountdown() {
        ...
        countdownText.textContent = 'GO!'; await new Promise(res => setTimeout(res, 500));
        countdownOverlay.classList.remove('active');
    }

    function showGameOver(score, game, mode) {
        Sound.stopMusic(); Sound.playSfx('gameOver');
        let newHigh = false;
        if (activeDifficulty === 'hard') { newHigh = HighScore.set(game, mode, activeDifficulty, score); }
    ...
    function animateScore(finalScore) { let currentScore=0; const inc=Math.max(1,Math.floor(finalScore/100)); const int=setInterval(()=>{currentScore+=inc; if(currentScore>=finalScore){currentScore=finalScore;clearInterval(int);} finalScoreElement.textContent=Math.round(currentScore);},10); }
    ...
    tetrisUI.backBtn.addEventListener('click', () => { if(activeGameModule) activeGameModule.stop(); showScreen('mainMenu'); });
    ...
    const Tetris = {
        ...
        init(mode, difficulty) {
            this.mode = mode; this.difficulty = difficulty; this.board = document.getElementById('tetris-board');
            ...
            this.createGridCells(); this.nextPiece = this.createPiece(); this.playerReset();
            const dropIntervals = {'easy': 1000, 'normal': 700, 'hard': 400}; this.dropInterval = dropIntervals[this.difficulty];
        ...
        createGridCells() { this.board.innerHTML = ''; this.cellElements = []; for(let i = 0; i < this.rows * this.cols; i++) { const cell = document.createElement('div'); this.cellElements.push(cell); this.board.appendChild(cell); }},
        draw() {
            ...
            for (let i = 0; i < this.cellElements.length; i++) {
                ...
                let newClassName = 'tetris-cell'; let newStyle = '';
        ...
        Object.assign(BlockPuzzle, {
            ...
            getDropCoordinates(draggedEl) { const boardRect=this.boardElement.getBoundingClientRect(); const dragRect=draggedEl.getBoundingClientRect(); const x = dragRect.left - boardRect.left + (dragRect.width / 2) - (this.touchOffsetX); const y = dragRect.top - boardRect.top + (dragRect.height / 2) - (this.touchOffsetY); const cellWidth=boardRect.width/10; return { row: Math.round(y/cellWidth), col: Math.round(x/cellWidth)}; },
