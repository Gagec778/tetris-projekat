document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- GLOBALNI ELEMENTI ---
    const screens = { mainMenu: document.getElementById('main-menu-screen'), gameMode: document.getElementById('game-mode-screen'), stats: document.getElementById('stats-screen'), game: document.getElementById('game-screen'), };
    ...
    const helpModal = document.getElementById('help-modal'); const statsBtn = document.getElementById('stats-btn');
    ...
    const puzzleUI = { score: document.getElementById('puzzle-score'), header: document.querySelector('.puzzle-only.game-header-bar'), pieceWrapper: document.getElementById('puzzle-pieces-container-wrapper'),};
    const tetrisUI = { header: document.getElementById('tetris-ui-header'), score: document.getElementById('tetris-score'), bestScore: document.getElementById('tetris-best-score'), level: document.getElementById('tetris-level'), nextPiece: document.getElementById('tetris-next-piece'), pauseBtn: document.getElementById('pause-btn-tetris'), backBtn: document.getElementById('back-btn-tetris'),};
    const gameAreas = { tetris: document.getElementById('tetris-game-area'), blockPuzzle: document.getElementById('block-puzzle-game-area'),};
    ...
    const Sound = {
        ...
        init() {
            try {
                ...
                this.sfx.hardDrop = new Audio('assets/sounds/hardDrop.mp3'); this.sfx.bomb = new Audio('assets/sounds/bomb.mp3'); this.music = new Audio('assets/sounds/music.mp3');
    ...
    const Settings = {
        ...
        init() {
            const savedTheme = localStorage.getItem('puzzleTheme') || 'dark'; const savedMusicVol = localStorage.getItem('puzzleMusicVol') || '0.2';
            const savedSfxVol = localStorage.getItem('puzzleSfxVol') || '0.3'; const savedHaptics = localStorage.getItem('puzzleHaptics') !== 'false';
            ...
            Sound.init(); this.setHaptics(savedHaptics);
            ...
            settingsBtn.addEventListener('click', () => settingsModal.classList.add('active')); this.closeBtn.addEventListener('click', () => settingsModal.classList.remove('active'));
        },
    ...
    const Stats = {
        ...
        render() { const container = document.querySelector('.stats-container'); const fastestSprint = this.get('highscore_tetris_sprint_hard', 'N/A'); container.innerHTML = `...`; /* Ostatak render funkcije */ }
    };
    const HighScore = {
    ...
    HighScore.updateDisplay(); Settings.init();
    ...
    function showModeSelect(gameType) {
        activeGame = gameType; const container = document.getElementById('mode-selection-container');
        document.getElementById('mode-select-title').textContent = gameType === 'tetris' ? 'Classic Blocks' : 'Puzzle Board';
        if (gameType === 'tetris') { container.innerHTML = `<div class="mode-btn" data-mode="marathon"><h3>Marathon (Classic)</h3>...</div> <div class="mode-btn" data-mode="sprint"><h3>Sprint</h3>...</div> <div class="mode-btn" data-mode="ultra"><h3>Ultra</h3>...</div>`; }
        else { container.innerHTML = `<div class="mode-btn" data-mode="classic"><h3>Classic</h3>...</div> <div class="mode-btn" data-mode="timeAttack"><h3>Time Attack</h3>...</div> <div class="mode-btn" data-mode="blast"><h3>Blast</h3>...</div>`; }
        showScreen('gameMode');
    }
    ...
    async function showFirstTimeHelp(game) { /* ... ista kao pre ... */ }
    ...
    function showGameOver(score, game, mode, time) {
        ...
        const title = gameOverModal.querySelector('#game-over-title'); title.textContent = newHigh ? 'NOVI REKORD!' : 'Kraj Igre';
    ...
    puzzleUI.header.querySelector('#pause-btn-puzzle').addEventListener('click', pauseGame);
    ...
    const fillFunctions = () => {
        Object.assign(Tetris, {
            ...
            startTimer() { let startTime = Date.now() - this.timer * 1000; this.timerInterval = setInterval(() => { if(!this.isPaused) { if(this.mode === 'sprint') { this.timer = ((Date.now() - startTime) / 1000); tetrisUI.score.textContent = `${this.timer.toFixed(2)}s`; } else if (this.mode === 'ultra') { this.timeLeft = 180 - ((Date.now() - startTime)/1000); tetrisUI.level.textContent = `${Math.ceil(this.timeLeft)}`; if(this.timeLeft <= 0) this.gameOver(); }}}, 100);},
            ...
            draw() { this.board.innerHTML = ''; const fullGrid = JSON.parse(JSON.stringify(this.boardState)); const ghost = {...this.player, matrix: JSON.parse(JSON.stringify(this.player.matrix)) }; while (!this.collide(this.boardState, ghost)) ghost.pos.y++; ghost.pos.y--; ghost.matrix.forEach((row, y) => { row.forEach((value, x) => { if (value !== 0) { const r = y + ghost.pos.y, c = x + ghost.pos.x; if(r >= 0 && r < this.rows && c >= 0 && c < this.cols) fullGrid[r][c] = 'ghost'; } }); }); this.player.matrix.forEach((row, y) => { row.forEach((value, x) => { if (value !== 0) { const r = y + this.player.pos.y, c = x + this.player.pos.x; if(r >= 0 && r < this.rows && c >= 0 && c < this.cols) fullGrid[r][c] = value; }}); }); fullGrid.forEach(row => { row.forEach(value => { const cell = document.createElement('div'); if (typeof value === 'number' && value !== 0) { cell.className = 'game-block'; const color = this.colors[value]; cell.style.backgroundColor = color; } else if (value === 'ghost') { cell.className = 'tetris-cell ghost'; } this.board.appendChild(cell); }); }); const nextDisplay = tetrisUI.nextPiece; nextDisplay.innerHTML = ''; nextDisplay.style.gridTemplateColumns = `repeat(${this.nextPiece[0].length}, 1fr)`; this.nextPiece.forEach(row => { row.forEach(value => { const cell = document.createElement('div'); cell.style.width = '15px'; cell.style.height = '15px'; if (value !== 0) { cell.className = 'game-block'; cell.style.backgroundColor = this.colors[value]; } nextDisplay.appendChild(cell); }); }); },
            ...
            arenaSweep() { let clearedRows = []; let perfectClear = true; for (let y = this.rows - 1; y >= 0; y--) { if (this.boardState[y].every(value => value !== 0)) { clearedRows.push(y); } else if (this.boardState[y].some(value => value !== 0)) { perfectClear = false; } } if (clearedRows.length > 0) { this.combo++; this.linesCleared += clearedRows.length; this.player.score += (([0, 100, 300, 500, 800][clearedRows.length] * this.level) + (this.combo * 50 * this.level)); Sound.playSfx('clear'); Sound.vibrate(50); Stats.add('tetris_linesCleared', clearedRows.length); clearedRows.reverse().forEach(y_index => { this.playLineClearEffect(y_index); this.boardState.splice(y_index, 1); this.boardState.unshift(new Array(this.cols).fill(0)); }); if(perfectClear) this.player.score += 10000; if (this.mode === 'marathon') this.level = Math.floor(this.linesCleared / 10) + 1; this.dropInterval = Math.max(100, (1000 - (this.level-1)*50) / (this.difficulty === 'hard' ? 2 : 1)); } else { this.combo = 0; } this.updateUI(); if (this.mode === 'sprint' && this.linesCleared >= 40) this.gameOver(); },
            playLineClearEffect(rowIndex) { /* ista kao pre */ },
            updateUI() { tetrisUI.score.textContent = this.player.score; tetrisUI.level.textContent = this.level; tetrisUI.bestScore.textContent = HighScore.get('tetris', this.mode); if(this.mode === 'sprint') { const linesLeft = 40 - this.linesCleared; tetrisUI.level.textContent = `${linesLeft > 0 ? linesLeft : 0}`; } tetrisUI.score.parentElement.classList.add('score-updated'); setTimeout(()=>tetrisUI.score.parentElement.classList.remove('score-updated'), 200); },
            playerReset() { this.player.matrix = this.nextPiece; this.nextPiece = this.createPiece(); this.player.pos.y = 0; this.player.pos.x = Math.floor(this.cols/2)-Math.floor(this.player.matrix[0].length/2); if(this.collide(this.boardState,this.player)) this.gameOver(); },
            ...
            gameOver() { this.stop(); let finalValue = this.mode === 'sprint' ? parseFloat(this.timer) : this.player.score; showGameOver(finalValue, 'tetris', this.mode, finalValue); },
        ...
        Object.assign(BlockPuzzle, {
            ...
            pieceShapes: [ {shape:[[1,1,1,1,1]]}, {shape:[[1,1],[0,1],[0,1]]}, {shape:[[1,1,1],[1,0,1]]}, {shape:[[1]]}, {shape:[[1,1,0],[0,1,1],[0,0,1]]}, {shape:[[1,1,1],[1,0,0],[1,0,0]]}, {shape:[[1,1,1,1],[0,0,0,1]]}, {shape:[[1,1],[1,1]]}, {shape:[[0,1,0],[1,1,1],[0,1,0]]}, {shape:[[1,1],[0,1]]}, {shape:[[1,1,1]]}, ],
            ...
            spawnBomb() { this.movesSinceBomb = 0; const filledCells = []; for(let r=0;r<10;r++){for(let c=0;c<10;c++){if(this.boardState[r][c] && !this.bombs.find(b=>b.r===r&&b.c===c)) filledCells.push({r,c});}} if(filledCells.length === 0) return; const cell = filledCells[Math.floor(Math.random()*filledCells.length)]; this.bombs.push({r: cell.r, c: cell.c, timer: 9}); Sound.playSfx('bomb'); this.renderBoard();},
            ...
            getHelpText() { return `<ul><li><strong>Classic:</strong> Opustite se i ređajte blokove dok imate mesta.</li><li><strong>Time Attack:</strong> Osvojite što više poena za 2 minuta.</li><li><strong>Blast:</strong> Preživite što duže! Povremeno se pojavljuju bombe koje morate očistiti pre isteka tajmera.</li></ul>`;}
        });
    }
    fillFunctions();
});
